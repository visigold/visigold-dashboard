    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-shrink-0">
          <div className="w-28 h-52 bg-gray-900 rounded-2xl p-1 shadow-lg relative">
            <div className="w-full h-full bg-white rounded-xl overflow-hidden flex flex-col">
              <div className="bg-[#1a3a6b] px-2 py-1.5">
                  ...
                  {(options.length > 0 ? options : [{ id: 0, label: "Accueil" }, { id: 1, label: "Rapidité" }, { id: 2, label: "Prix" }]).slice(0, 3).map((opt, i) => (
                    <div
                      ...
                      className={`text-[5px] px-1.5 py-0.5 rounded text-white font-medium ${
                        i === 0 ? "bg-[#1a3a6b]" : i === 1 ? "bg-blue-400" : "bg-blue-300"
                      }`}
                    >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-gray-600 mb-1">Modifier la question</p>
          <p className="text-[10px] text-gray-400 mb-1">Question actuelle :</p>
          <textarea
            value={questionText}
            onChange={(e) => setQuestionText(e.target.value)}
            className="w-full text-xs border border-gray-200 rounded-lg p-2 resize-none focus:outline-none focus:ring-2 focus:ring-[#1a3a6b] text-gray-700"
            rows={3}
            placeholder="Texte de la question..."
          />

          <p className="text-[10px] text-gray-400 mt-2 mb-1">Réponses :</p>
            ...
            {options.map((opt, i) => (
              <div key={opt.id} className="flex items-center gap-1.5">
                ...
                <input
                  ...
                  className="flex-1 text-xs border border-gray-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-[#1a3a6b]"
                />
